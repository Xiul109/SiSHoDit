...
var _timestampt : float = 0

# Called when the node enters the scene tree for the first time.
func _ready():
	simulated.connect(_on_simulated)
	_timestampt += Time.get_unix_time_from_datetime_dict({"year":year, "month":month, "day":day,
...
func _on_simulated(delta):
	_timestampt += delta
	var time_dict = Time.get_datetime_dict_from_unix_time(_timestampt)
	time_dict["second"] += fmod(_timestampt, 1.0)
