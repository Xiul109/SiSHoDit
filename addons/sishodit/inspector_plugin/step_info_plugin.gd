...
var NeedDictEditor = preload("res://addons/sishodit/inspector_plugin/ui/need_dict_editor.gd")
var GroupStringEditor = preload("res://addons/sishodit/inspector_plugin/ui/group_string_editor.gd")
...
func _parse_property(object, type, name, hint, hint_text, usage, wide):
	if type == TYPE_DICTIONARY and name.contains("need"):
		add_property_editor(name, NeedDictEditor.new(object[name], _get_edited_needs_keys()))
		return true
	if name == "object_group" and type == TYPE_STRING:
