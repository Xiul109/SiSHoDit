...
signal updated(state, delta) # Time to update(based checked yafsm_process_mode), up to user to handle any logic, for example, update movement of CharacterBody3D

# Enum to define how state machine should be updated
enum YAFSMProcessMode {
	PHYSICS,
	IDLE,
	MANUAL
}
...
@export var active: bool = true : set = set_active # Activeness of player
@export var autostart: bool = true # Automatically enter Entry state checked ready if true
@export var yafsm_process_mode: YAFSMProcessMode = YAFSMProcessMode.IDLE : set = set_yafsm_process_mode # ProcessMode of player
...
func _init():
	...
	_parameters = {}
	_local_parameters = {}
	_was_transited = true # Trigger _transit checked _ready
...
func _initiate():
	if autostart:
		start()
	_on_active_changed()
	_on_yafsm_process_mode_changed()
...
func _transit():
	...
	if next_state:
		if stack.has(next_state):
			...
			push(next_state)
	var to = next_state
	_was_transited = next_state != null
...
func _on_state_changed(from, to):
	match to:
		State.ENTRY_STATE:
			emit_signal("entered", "")
		State.EXIT_STATE:
			set_active(false) # Disable checked exit
			emit_signal("exited", "")
...
# Called internally if yafsm_process_mode is PHYSICS/IDLE to unlock update()
func _update_start():
	_is_update_locked = false

# Called internally if yafsm_process_mode is PHYSICS/IDLE to lock update() from external call
func _update_end():
	_is_update_locked = true

# Called after update() which is dependant checked yafsm_process_mode, override to process current state
func _on_updated(delta, state):
	pass

func _on_yafsm_process_mode_changed():
	if not active:
		return

	match yafsm_process_mode:
		YAFSMProcessMode.PHYSICS:
			set_physics_process(true)
			set_process(false)
		YAFSMProcessMode.IDLE:
			set_physics_process(false)
			set_process(true)
		YAFSMProcessMode.MANUAL:
			set_physics_process(false)
			set_process(false)
...
# Trigger another call of _flush_trigger checked first layer of dictionary if nested is true
...
func reset(to=-1, event=ResetEventTrigger.LAST_TO_DEST):
	...
	_was_transited = true # Make sure to call _transit checked next update
...
# Update player to, first initiate transition, then call _on_updated, finally emit "update" signal, delta will be given based checked yafsm_process_mode.
# Can only be called manually if yafsm_process_mode is MANUAL, otherwise, assertion error will be raised.
# *delta provided will be reflected in signal updated(state, delta)
func update(delta=get_physics_process_delta_time()):
	if not active:
		return
	if yafsm_process_mode != YAFSMProcessMode.MANUAL:
		assert(not _is_update_locked) #,"Attempting to update manually with ProcessMode.%s" % ProcessMode.keys()[yafsm_process_mode])
...
# Set trigger to be tested with condition, then trigger _transit checked next update, 
# automatically call update() if yafsm_process_mode set to MANUAL and auto_update true
...
# Set param(null value treated as trigger) to be tested with condition, then trigger _transit checked next update, 
# automatically call update() if yafsm_process_mode set to MANUAL and auto_update true
...
# Remove param, then trigger _transit checked next update, 
# automatically call update() if yafsm_process_mode set to MANUAL and auto_update true
# Nested param can be accessed through path "path/to/param_name", for example, "App/Game/is_playing"
...
# Clear params from specified path, empty string to clear all, then trigger _transit checked next update, 
# automatically call update() if yafsm_process_mode set to MANUAL and auto_update true
# Nested param can be accessed through path "path/to/param_name", for example, "App/Game/is_playing"
...
# Called when param edited, automatically call update() if yafsm_process_mode set to MANUAL and auto_update true
func _on_param_edited(auto_update=true):
	_is_param_edited = true
	if yafsm_process_mode == YAFSMProcessMode.MANUAL and auto_update and _is_started:
		update()
...
func set_yafsm_process_mode(mode):
	if yafsm_process_mode != mode:
		yafsm_process_mode = mode
		_on_yafsm_process_mode_changed()
