...
var editor_accent_color = Color.WHITE : set = set_editor_accent_color
...
func _init():
	add_child(tween)
	tween.connect("tree_entered",Callable(self,"_on_tween_tree_entered"))
	...
	for transition in transitions.values():
		...
		if line:
			...
			for condition in transition.conditions.values():
				...
				line.update_label()
				# Condition label color based checked comparation
...
func debug_transit_out(from, to):
	...
	for transition in transitions.values():
		...
		if line:
			...
			line.update_label()
			tween.remove_at(line, "self_modulate")
