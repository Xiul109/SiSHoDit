...
func _ready():
	...
	name_edit.connect("focus_exited",Callable(self,"_on_NameEdit_focus_exited"))
	name_edit.connect("text_submitted",Callable(self,"_on_NameEdit_text_entered"))
	set_process_input(false) # _input only required when name_edit enabled to check mouse click outside

func _draw():
	if state is StateMachine:
		if selected:
			draw_style_box(get_stylebox("nested_focus", "StateNode"), Rect2(Vector2.ZERO, size))
		else:
			draw_style_box(get_stylebox("nested_normal", "StateNode"), Rect2(Vector2.ZERO, size))
	else:
		super._draw()
...
func _on_state_changed(new_state):
	if state:
		state.connect("name_changed",Callable(self,"_on_state_name_changed"))
		if name_edit:
