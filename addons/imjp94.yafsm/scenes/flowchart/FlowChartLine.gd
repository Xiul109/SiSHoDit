...
func _draw():
	...
	if selected:
		...
		draw_style_box(get_stylebox("focus", "FlowChartLine"), Rect2(Vector2.ZERO, size))
	else:
		draw_style_box(get_stylebox("normal", "FlowChartLine"), Rect2(Vector2.ZERO, size))
...
func join(from, to, offset=Vector2.ZERO, clip_rects=[]):
	...
	rotation = rad_to_deg(Vector2.RIGHT.angle_to(dir))
	pivot_at_line_start()
