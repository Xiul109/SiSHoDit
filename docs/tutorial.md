...
* The **Simulable** is a type of node that must be attached to every node that require active simulation, which includes Agents and Sensors mainly, but also other special nodes that will be explained in deep later. An example of node that shouldn't need to be Simulable is a wall, for example, given that it is completly static and not sensorized in any way. The Simulator only works directly with Simulable objects, and the parents of those comunicate with the Simulator through their childs.

In essence those two are the main tools of the simulator and any user could implement any other part from those, however, the addon includes a lot of tools to set up an environment without needing to add a single line of code.
...
As can be noted, the walls are not a exact match for this example. Thats mainly because is difficult to adjust so well with the models used, but depending of your needs you can used more appropiated assets. The next step consist of finishing the structure by adding the floor and the doors.

![Structure](figures/structure.png)

### Furnitures structure

Now, the next step is adding furnitures, but before that, let's dig into a furniture structure. The picture below shows the node tree of a toilet. 

![Furniture structure](figures/furniture_structure.png)

Next, each part of the node tree is explained more in deep:

* **toilet (StaticBody3D)**. The root is a static body to handle collisions. It could be any type of Node3D, but many times it is needed to handle collisions, so typically StaticBody3D are prefered. It is relevant that it belongs to a group, which, despite is not showed in the picture, is called also "toilet". Every furniture that can be actively used by an Agent, must belong to one or more groups that informs about the utility of the object.
...
### Usable Nodes
When an Usable node is child of an object, it allows a specific way of using that object defined by the Usable node itself. Usable objects have three main steps when used: start using, being used, and finish using. There are multiple Usable nodes predefined in the addon, but the user can create their own ones by creating a script inhereting from Usable and overriding its methods. The predefined Usables are stored as scenes that can be found in *res://addons/sishodit/presets/usable_nodes/* and the list can be seen below.

* **BasicUsable** produces a value through a sensor when the agent starts using it, and another one when it finishes. Both values can be parametrizables by the user.
* **SingleTriggerUsable** is only triggered once, at the beggining of using it or at the end, which is decided by the user.
...
### Sensors
A type of node that represents a sensor deployed in the smart environment. They have Simmulable childs, so they are considered Simulable entities. It has the capability of managing uncertainty by producing wrong triggers if the user parametrized it that way. The main parameters of a sensor are the next ones:
* **Sensor Name** is the name of the sensor that will appear in the output.
* **Sensor Type** is also information that appears in the output.
...
Besides those, there are also parameteres to simulate malfunction of uncertainty:
...
* **Average Time Between Wrong Triggers** and **STD Time Between Wrong Triggers** are the mean and standar deviation of a normal distribution from which the time that will pass until the sensor is triggered when it should not is obtained.
  
### Placing furnitures
In this step the house will be completed by adding the remaining furnitures as depicted in the image. Most of the furnitures are built with a structure similar to the toilet example of before. **It is important to remember that every object with a function in the needs of an Agent, should belong to a group identifying the need**. The final distribution is a bit diferent to the blueprint to adjust better the space.
...
The Agent is the main actuator in the simulation. It is the one that interacts with objects and sensors to trigger them. In this section, we are not going to deepen on the node structure of the Agent, but we are going to focus in deep on how its behaviour can be configured. By default, the Agent model is a green capsule, but you can change it if prefered.
 ...
 * **Solutions**. The list of posible solutions for the need. They can be added in a similar way than needs to the agent. In the next section we will see the characteristics of solutions.
 * **Time to Fill Level**. This is the time in seconds needed until the need is completly filled.
...
### Solutions
Solutions are series of steps that can be used to solve a need. Between all the solutions available for a need one is randomly chosed depending on its weight.
 ...
 * **Weight**. Increasing the weight, makes the chances of this solution to be chosen over other ones. For example, in a need with 3 solutions, *sol1* and *sol2* with a weigth of 1 and *sol3* with a weight of 2, the probabilities of chosing each of them will be the next: *sol1* - 25% (1/4), *sol2* - 25% (1/4), *sol3* - 50% (2/4).
 * **Condition Manager**. A condition manager can be used to add prerequisites for a solution to be pickable in base of a context, for example the hour of the day. More will be seen about context and conditions in their own section.
 * **Resource Name**. It is always good to add a name for the solution, cause it will help in debuging and for the output file.
...
* **Duration.** This group is composed by the two parameters below, along with an alias that will be used for refering to them. These parameters are used to obtain the time needed for performing the step. The value is randomly chosen from a normal distribution with a mean of *(m + M) / 2* and a standar deviation of *(M - m) / 8* and clamped between *m* and *M*. 
  * **Min Duration**: *m*.
  * **Max Duration**: *M*.
* **Needs.** This group stablishes how steps interacts with the needs. All of them include a text field and a slider for each need you need to affect. The text field is used for refering to the need key that this step will affect, while the slider moves from 0 to 1 for defining how much the value will be affected.
...
In the tutorial scenes you can see the final behaviour settings for the tutorial Agent.
...
The example above shows two conditions related to the *hour* key, but only one needs to be evaluated as true, being the final condition: *hour* >= 22 or *hour* <= 6. That delimites the range between 10:00 PM and 6:00 AM.

## Executing the simulation
Ok, so now we have our simulation completly set up, how we run it? There are 3 different ways to do it. 
